...
const CardDetail: React.FC<Props> = ({ title, images, description }) => {
    gsap.registerPlugin(TextPlugin);
    gsap.registerPlugin(ScrollTrigger);
    gsap.registerPlugin(useGSAP);
   
    const animTitle = useRef<HTMLHeadingElement | null>(null); 
    ...
    const closeSlider = () => {
      ...
      return (
        ...
        <div className={`${styles.card__detail} padding`}>
          <h1 ref={animTitle}>{title && title}</h1>
          <div className={styles.photo_grid}>
            {images && images.map((img, index) => (
              <div className={styles.image_wrap} key={index}>
                <Image
                /** тут пока под вопросом */
                  ref={(el) => {
                    if (el) imgArr.current[index] = el;
                  }}
          ...
          {isSliderOpen && (
            <div className={styles.slider_overlay}>
              ...
              <PortfolioCarousel slides={images && images} initialIndex={currentIndex} />
            </div>
          )}
      </div>
